...
// ETAPA 36: antes de abrir WhatsApp presento la invitación oficial de preinauguración.
const INAUGURATION_GROUP='https://chat.whatsapp.com/FUaa4tGIDOTBeqJOMmbrJT';
function launchSelectionMessage(){
  const rows=selectionRows();
  const lines=rows.map(({product,qty})=>`• ${qty} × ${product.name} (${product.brand})`);
  return `Hola LIHEN.CO ✨\n\nMe interesan estos productos:\n${lines.join('\n')}\n\nTotal de unidades: ${totalUnits()}.\n\nTambién quiero recibir información sobre la inauguración del 2 de agosto de 2026. Autorizo el contacto por WhatsApp relacionado con esta consulta y con la inauguración.`;
}
function mountPrelaunchModal(){
  if(document.querySelector('[data-prelaunch-modal]'))return;
  const host=document.createElement('div');
  host.className='prelaunch-backdrop';
  host.dataset.prelaunchModal='';
  host.hidden=true;
  host.innerHTML=`<section class="prelaunch-modal" role="dialog" aria-modal="true" aria-labelledby="prelaunch-title"><button class="prelaunch-close" type="button" data-prelaunch-close aria-label="Cerrar">×</button><div class="prelaunch-art"><span>02</span><small>AGOSTO · 2026</small><div><b>Presencial</b><i>+</i><b>Virtual</b></div></div><div class="prelaunch-copy"><p class="eyebrow">Tu selección está lista</p><h2 id="prelaunch-title">Antes de continuar, queremos invitarte a nuestra inauguración.</h2><p>LIHEN.CO abrirá oficialmente el <strong>2 de agosto de 2026</strong>. Puedes conocer la experiencia, unirte voluntariamente al grupo oficial o enviar tu consulta por WhatsApp.</p><div class="prelaunch-selection" data-prelaunch-summary></div><div class="prelaunch-actions"><a class="btn btn-lilac" href="./inauguracion/">Conocer la inauguración</a><a class="btn btn-light" href="${INAUGURATION_GROUP}" target="_blank" rel="noopener">Unirme al grupo oficial</a><button class="btn btn-dark" type="button" data-prelaunch-whatsapp>Enviar consulta por WhatsApp</button><button class="prelaunch-secondary" type="button" data-prelaunch-close>Seguir explorando</button></div><small class="prelaunch-consent-note">Entrar al grupo es voluntario. La consulta directa incluye una autorización breve para recibir información relacionada con tu selección y la inauguración.</small></div></section>`;
  document.body.append(host);
  host.addEventListener('click',event=>{
    if(event.target===host||event.target.closest('[data-prelaunch-close]'))closePrelaunchModal();
    if(event.target.closest('[data-prelaunch-whatsapp]')){
      window.open(`https://wa.me/573058947808?text=${encodeURIComponent(launchSelectionMessage())}`,'_blank','noopener');
    }
  });
}
function openPrelaunchModal(){
  ...
  mountPrelaunchModal();
  closeDrawer();
  const host=document.querySelector('[data-prelaunch-modal]');
  const summary=host?.querySelector('[data-prelaunch-summary]');
  if(summary){const rows=selectionRows();summary.innerHTML=`<strong>${rows.length} referencia${rows.length===1?'':'s'} · ${totalUnits()} unidad${totalUnits()===1?'':'es'}</strong><span>${rows.slice(0,3).map(({product})=>safe(product.name)).join(' · ')}${rows.length>3?' · …':''}</span>`;}
  if(host){host.hidden=false;document.body.classList.add('no-scroll');requestAnimationFrame(()=>host.classList.add('is-open'));host.querySelector('.prelaunch-close')?.focus();}
}
function closePrelaunchModal(){
  ...
  setTimeout(()=>{host.hidden=true},180);
}
document.addEventListener('click',event=>{if(event.target.closest('[data-prelaunch-open]')){event.preventDefault();openPrelaunchModal();}});
