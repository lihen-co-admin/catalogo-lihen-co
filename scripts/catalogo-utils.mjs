...
export function categoryFolderFromImages(images) {
  ...
  const match = first.match(
    /^\.\/assets\/productos\/(?:STYLE|BEAUTY_CARE)\/([^/]+)\//
  );
...
export function discoverImages(
  ...
  return fs
    ...
    .map(
      (name) =>
        `./assets/productos/${lineFolder(
          line
        )}/${categoryFolder}/${productFolder}/${name}`
    );
}
...
export async function loadProducts() {
  ...
  if (!Array.isArray(mod.products)) {
    throw new Error(
      'products.js no exporta un arreglo products válido.'
    );
  }
